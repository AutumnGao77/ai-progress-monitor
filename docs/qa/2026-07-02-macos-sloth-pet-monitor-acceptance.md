...
2026-07-21 至 2026-07-29，系统通知开关增量完成开发态自动化、浏览器和 macOS Dev App 实机验收；通知开关、多窗口聚焦、残留会话清理和多工具状态回归修复随 PR #5 合并到 `main`（`770d447`）。该能力不属于 v0.2.1，现进入 v0.3.0 发布候选流程。
...
| 项目 | 结论 |
|---|---|
| 当前本地源码状态 | PR #5 与 PR #6 已合并到 `main`；v0.3.0 发布候选分支正在执行独立发布门禁 |
| 自动化测试 | v0.2.1 发布基线 445 项均通过；v0.3.0 portable 阻断修复后已独立取得 `517 tests OK` 和 `release-validation-ok` |
| Swift 编译 | 通过 |
...
| 真实运行 | macOS 开发态 App 已多轮真实测试；v0.3.0 最终 ZIP 内 App 已完成首次启动、通知开关、重启保持、外观、三态、隐藏/恢复和退出验收，GitHub 回下载附件仍待发布后复核 |
| 完整目标状态 | v0.2.1 历史发布已验收；v0.3.0 完成候选包、Tag、Release 与回下载校验后才能判定发布完成 |
